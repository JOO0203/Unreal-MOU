...
작성: 2026-08-12
최종 갱신: 2026-08-18 (V4 지터버퍼/PLC 구현 — 14-6절 신설)
상태:
...
- **V3**(RPC + 서버 라우팅 + 3D 재생) 구현 완료 + 빌드 통과 (2026-08-18).
  ★ **PIE 2창 실행 검증은 아직 안 했다**(14-5절 마지막 항목).
  루프백은 V1/V2 경로라 네트워크 라우팅을 거치지 않는다 — 별개의 검증이다.
- **V4**(지터버퍼 + PLC) 구현 완료, **빌드 검증 전** (2026-08-18, 14-6절).
- V5 이후는 설계만.

...
void AMainCharacter::BeginExternalGrab(USceneComponent* GrabParent, FName SocketName,
	FVector LocationOffset, FRotator RotationOffset)
{
	if (!IsValid(GrabParent))
	{
		return;
	}
	...
	if (!HasAuthority())
	{
		ServerBeginExternalGrab(GrabParent, SocketName, LocationOffset, RotationOffset);
		return;
	}

	MulticastBeginExternalGrab(GrabParent, SocketName, LocationOffset, RotationOffset);
}

void AMainCharacter::ServerBeginExternalGrab_Implementation(USceneComponent* GrabParent, FName SocketName,
	FVector LocationOffset, FRotator RotationOffset)
{
	BeginExternalGrab(GrabParent, SocketName, LocationOffset, RotationOffset);
}

void AMainCharacter::MulticastBeginExternalGrab_Implementation(USceneComponent* GrabParent, FName SocketName,
	FVector LocationOffset, FRotator RotationOffset)
{
	if (!IsValid(GrabParent) || bExternalGrabActive)
	{
		return;
	}

	bExternalGrabActive = true;
	PreGrabActorRotation = GetActorRotation();
	bHadPreGrabController = IsValid(GetController());
	if (bHadPreGrabController)
	{
		PreGrabControlRotation = GetController()->GetControlRotation();
	}

	if (USkeletalMeshComponent* PlayerMesh = GetMesh())
	{
		PreGrabMeshRelativeTransform = PlayerMesh->GetRelativeTransform();
		PreGrabMeshAttachSocket = PlayerMesh->GetAttachSocketName();
	}

	if (UCapsuleComponent* Capsule = GetCapsuleComponent())
	{
		PreGrabCapsuleCollision = static_cast<uint8>(Capsule->GetCollisionEnabled());
		Capsule->SetCollisionEnabled(ECollisionEnabled::NoCollision);
	}

	if (UCharacterMovementComponent* Movement = GetCharacterMovement())
	{
		PreGrabMovementMode = static_cast<uint8>(Movement->MovementMode);
		PreGrabCustomMovementMode = Movement->CustomMovementMode;
		Movement->StopMovementImmediately();
		Movement->DisableMovement();
	}

	AttachToComponent(GrabParent, FAttachmentTransformRules::SnapToTargetNotIncludingScale, SocketName);
	SetActorRelativeLocation(LocationOffset);
	SetActorRelativeRotation(RotationOffset);
}

void AMainCharacter::ReleaseExternalGrab(bool bThrow, FVector ThrowVelocity)
{
	if (!HasAuthority())
	{
		ServerReleaseExternalGrab(bThrow, ThrowVelocity);
		return;
	}

	RemoveExternalGrabTags();
	MulticastReleaseExternalGrab(bThrow, ThrowVelocity);
}

void AMainCharacter::ServerReleaseExternalGrab_Implementation(bool bThrow, FVector ThrowVelocity)
{
	ReleaseExternalGrab(bThrow, ThrowVelocity);
}

void AMainCharacter::MulticastReleaseExternalGrab_Implementation(bool bThrow, FVector ThrowVelocity)
{
	DetachFromActor(FDetachmentTransformRules::KeepWorldTransform);

	if (USkeletalMeshComponent* PlayerMesh = GetMesh())
	{
		PlayerMesh->AttachToComponent(GetCapsuleComponent(), FAttachmentTransformRules::KeepRelativeTransform,
			PreGrabMeshAttachSocket);
		PlayerMesh->SetRelativeTransform(PreGrabMeshRelativeTransform);
	}
	...
	if (UCameraComponent* Camera = GetFollowCamera())
	{
		...
		Camera->SetRelativeRotation(FirstPersonCameraRotation);
		Camera->bUsePawnControlRotation = true;
	}

	SetActorRotation(PreGrabActorRotation);
	if (bHadPreGrabController && IsValid(GetController()))
	{
		GetController()->SetControlRotation(PreGrabControlRotation);
	}

	if (UCapsuleComponent* Capsule = GetCapsuleComponent())
	{
		Capsule->SetCollisionEnabled(static_cast<ECollisionEnabled::Type>(PreGrabCapsuleCollision));
	}
	...
	if (UCharacterMovementComponent* Movement = GetCharacterMovement())
	{
		if (bThrow)
		{
			...
			LaunchCharacter(ThrowVelocity, true, true);
		}
		else
		{
			const EMovementMode RestoredMode = PreGrabMovementMode == static_cast<uint8>(MOVE_None)
				? MOVE_Walking
				: static_cast<EMovementMode>(PreGrabMovementMode);
			Movement->SetMovementMode(RestoredMode, PreGrabCustomMovementMode);
		}
	}

	bExternalGrabActive = false;
}

void AMainCharacter::RemoveExternalGrabTags()
{
	...
	FGameplayTagContainer HeldTags;
	const FGameplayTag PlayerHeldTag = FGameplayTag::RequestGameplayTag(FName("State.Player.Held"), false);
	const FGameplayTag HeldTag = FGameplayTag::RequestGameplayTag(FName("State.Held"), false);
	if (PlayerHeldTag.IsValid())
	{
		HeldTags.AddTag(PlayerHeldTag);
		AbilitySystemComponent->RemoveLooseGameplayTag(PlayerHeldTag);
	}
	if (HeldTag.IsValid())
	{
		HeldTags.AddTag(HeldTag);
		AbilitySystemComponent->RemoveLooseGameplayTag(HeldTag);
	}

	if (!HeldTags.IsEmpty())
	{
		AbilitySystemComponent->RemoveActiveEffectsWithGrantedTags(HeldTags);
	}
}
